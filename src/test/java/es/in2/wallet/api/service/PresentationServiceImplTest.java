...
class PresentationServiceImplTest {
    ...
    @Test
    void createSignedTurnstileVerifiablePresentation_UserExists_ReturnsSignedTurnstileVP() throws JsonProcessingException {
        ...
        String vpClaims = """
            {
              ...
              "verifiableCredential": ["dummyVC.jwt.token"],
        ...
        VerifiableCredential credential = VerifiableCredential.builder()
                .id("vc1")
                .type(List.of("LEARCredential"))
                .build();

        try (MockedStatic<ApplicationUtils> ignored = Mockito.mockStatic(ApplicationUtils.class)) {
            when(getUserIdFromToken(authorizationToken)).thenReturn(Mono.just(userId));
            when(credentialService.getCredentialDataByIdAndUserId(processId, userId, credential.id()))
                    .thenReturn(Mono.just(vcJwt));
            when(objectMapper.writeValueAsString(any())).thenReturn(vpClaims);
            when(objectMapper.readTree(anyString())).thenReturn(new ObjectMapper().readTree(vpClaims));
            when(signerService.buildJWTSFromJsonNode(any(), anyString(), anyString()))
                    .thenReturn(Mono.just(signedVP));

            StepVerifier.create(presentationService.createSignedTurnstileVerifiablePresentation(processId, authorizationToken, credential, nonce, audience)).expectNext(encodedVP).verifyComplete();
        }
    }
